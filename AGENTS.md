...
本仓库是 Cursor 开发环境配置中枢；知识库覆盖 **Graphify 图谱**、**Headroom Token 压缩**、**Agency Agents**、**Caveman / Ponytail / UZI / Serenity / Bottleneck Hunter / Serenity-Bottleneck-Hunter Skills**、**Notion MCP**、**agentmemory**、**长篇小说连续性（novel-continuity）**、**Cursor Agent API（AstrBot proxy / Cloud REST / SDK）** 等工具链。
...
5. 运行 `graphify update .` 刷新 `graphify-out/`（AST，无 API 成本）
6. 大量文档变更后运行 `graphify extract . --cluster-only` 全量重建
...
3. `wiki/index.md` → 主题页
4. 原始文件 `HEADROOM.md` / `GRAPHIFY.md` / `NOTION-MCP.md` / `AGENTMEMORY.md` / `NOVEL-WRITING.md` / `CURSOR-AGENT-API.md`
