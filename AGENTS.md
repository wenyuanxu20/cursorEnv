...
5. 改 Firecrawl / TrendRadar / Scrapling 自托管、MCP 或联网取数规则后，同步 `wiki/firecrawl/`、`wiki/trendradar/`、`wiki/scrapling/` 与根目录 `FIRECRAWL.md`、`TRENDRADAR.md`、`SCRAPLING.md`
6. 运行 `graphify update .` 刷新 `graphify-out/`（AST，无 API 成本）
7. 大量文档变更后运行 `graphify extract . --cluster-only` 全量重建
8. 改 wiki / raw / schema / AGENTS / 小说 canon 后，同步到 `C:\Users\xwy12\Desktop\my-project\knowledge-hub`（`knowledge-hub/sync.ps1 -Project cursorEnv`）；跨项目总览见该目录 `INDEX.md`
...
2. 需要从**网络**取信息时：热榜走 TrendRadar（全局 `firecrawl-web-fetch.mdc`；MCP `:3333`；键 `trendradar`），具体 URL/正文走本机 Firecrawl（API `:3002`；MCP：`user-firecrawl`），反爬或 Firecrawl 失败走 Scrapling（STDIO MCP 键 `scrapling`），不要先走 WebFetch
...
5. 原始文件 `HEADROOM.md` / `GRAPHIFY.md` / `NOTION-MCP.md` / `AGENTMEMORY.md` / `FIRECRAWL.md` / `TRENDRADAR.md` / `SCRAPLING.md` / `NOVEL-WRITING.md` / `CURSOR-AGENT-API.md`
...
- 介绍文档：仓库根 `README.md` 与 `cursor-env-manifest.json` 须覆盖当前工具链（含 agentmemory、Firecrawl、TrendRadar、Scrapling）；静态页 `web/index.html`
